...
def main():
    ...
    if args.synthetic:
        ...
        print('running truth backtest...', flush=True)
        res = bt.run(data, funding)
        print(f'truth run done: {len(res.trades)} trades — writing report...', flush=True)
        ...
        print('running lookahead audit (3 shifted runs)...', flush=True)
        la = lookahead_audit(bt, data, funding)
        print('running recursive audit (3 warmup variants)...', flush=True)
...
if __name__ == '__main__':
    try:
        ...
        import traceback
        OUT.mkdir(parents=True, exist_ok=True)
        (OUT / 'crash.log').write_text(traceback.format_exc())
        traceback.print_exc()
        print(f'\n💥 gate crashed — full traceback saved to {OUT / "crash.log"}')
        raise SystemExit(1)
